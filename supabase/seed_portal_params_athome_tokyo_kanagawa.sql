-- seed_portal_params_athome_tokyo_kanagawa.sql
-- athome portal_area_params の補完
...
--
-- 旧マスターで確認済みの市区スラッグを参照して命名（verified=true）
-- 旧マスターにない市・駅は推測スラッグ（verified=false）

-- ============================================================
-- 東京都 市部・郡部（athome city path）
-- ============================================================
INSERT INTO portal_area_params (area_id, portal, param_type, portal_code, portal_url_param, verified, notes)
SELECT am.id, 'athome', 'city_path', slug, '/tokyo/' || slug, true, 'athomeシティパス（推測・要確認）'
FROM (VALUES
  ('八王子市',   'hachioji-city'),
  ('立川市',     'tachikawa-city'),
  ('武蔵野市',   'musashino-city'),
...
WHERE NOT EXISTS (
  ...
  WHERE p.area_id = am.id AND p.portal = 'athome'
)
ON CONFLICT DO NOTHING;

-- ============================================================
-- 神奈川県 郡部（athome city path）
-- ============================================================
INSERT INTO portal_area_params (area_id, portal, param_type, portal_code, portal_url_param, verified, notes)
...
-- ============================================================
-- 東京都 主要新規駅（athome station_path 推測URL、verified=false）
-- ============================================================
INSERT INTO portal_area_params (area_id, portal, param_type, portal_code, portal_url_param, verified, notes)
...
FROM (VALUES
  ...
  ('国立競技場', 'kokuritsukyogijo-station'),
...
-- ============================================================
-- 神奈川県 主要新規駅（athome station_path 推測URL、verified=false）
-- ============================================================
INSERT INTO portal_area_params (area_id, portal, param_type, portal_code, portal_url_param, verified, notes)
